...
$.fn.areaSmall = function (options) {
    ...
    var provDom = _this.siblings(opts.provDom);
    var cityDom = _this.siblings(opts.cityDom);

    var SetValue = function () {
        _this.text(opts.defaultText);
        if(provDom.val()) {
            var text = provDom.val() + (cityDom.val()? "，" + cityDom.val(): "" );
            _this.text(text);
        }
    }
    SetValue();

    var provhint = $('<div class="provhint"><a class="close_prov"></a></div>').appendTo("body");
    _.each(exports.cityHintArea, function (v, k) {
    ...
    var cityhint = $("<ul>", { "class": "cityhint" }).appendTo(provhint);
    cityhint.on("click", "a", function () {
        cityDom.val($(this).text())
        $('a.close_prov', provhint).click();
    });
    $("a.prov", provhint).on("click", function () {
        var _prov = $(this);
        provDom.val(_prov.text());
        cityDom.val('');
        var cm = exports.cityHintProv[_prov.text()];
        if(cm.length < 2) {
            $('a.close_prov', provhint).click();
            return;
        }
