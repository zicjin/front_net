    ...
    var Select = Overlay.extend({
        ...
        setup: function() {
            ...
            var trigger = this.get("trigger").on("click", {
                self: this
            }, this._trigger_click).on("mouseenter", function(e) {
        ...
        removeOption: function(option) {
            var removedIndex = getOptionIndex(option, this.options, this.get("multiple")), oldIndex = this.get("selectedIndex"), //removedOption = this.options.eq(removedIndex);
        ...
        _onRenderSelectedIndex: function(index) {
            ...
            */
            ...
            var currentItem = this.currentItem, value = selected.attr("data-value");
            // 如果两个 DOM 相同则不再处理
            ...
            if (currentItem && this.element.has(currentItem)) {
                currentItem.attr("data-selected", "false").removeClass(this.get("classPrefix") + "-item-selected");
                var m = getModelByIndex(getOptionIndex(currentItem, this.options, this.get("multiple")), this.model.options);
                m && (m.selected = "false");
            }
            // 处理当前选中的元素
            selected.attr("data-selected", "true").addClass(this.get("classPrefix") + "-item-selected");
            ...
            if (source) {
                ...
                source.val(value);
                if (value !== old_val) $(source).trigger("change");
            }
